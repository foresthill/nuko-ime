...
📖 **詳しい使い方は [使い方マニュアル (docs/MANUAL.md)](docs/MANUAL.md) を参照。**
特に **「ん」はアポストロフィで明示**(範囲=`han'i`、個人用=`kojin'you`)など、
知っておくと詰まらないコツをまとめています。
...
| ↓ / ↑ | 次候補 / 前候補 |
| 数字 1-9 | 該当行の候補を直接確定(表示中ページ内) |
| Enter | 現在の選択を確定 |
| Escape | 変換をキャンセル |
| Backspace | 1 文字削除 / 変換中は未変換に戻す |
| ← / → | **文節フォーカスを移動**(複数文節時) |
| Shift+← / Shift+→ | **文節の区切りを伸縮**(右端が動く) |

> ネイティブアプリ(メモ / テキストエディット / Safari 等)で快適に動きます。
> Electron/Chromium 系(Claude Code / VS Code / Slack 等)ではハイライト・
> ポップアップ位置に制約があります(アプリ側の制約)。
