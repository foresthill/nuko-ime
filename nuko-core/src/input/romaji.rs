...
mod tests {
    ...
    ///
    /// ## `nn` 規約と「ライブラリを使わない」判断 (2026-09-16 実測で確定)
    ///
    /// - `hanni` → **はんに** が本 IME の規約。「範囲(はんい)」は `han'i` で打つ。
    /// - これは Mozc 系 (本実装) と WanaKana (`wana_kana` crate) の **独立 2 実装が
    ///   一致** する挙動。`hanni→はんい` は ことえり固有の少数派挙動。
    /// - **ライブラリ (`wana_kana` 5.0) への置換は却下**。実測で `hanni` は同じ「はんに」
    ///   な上、`konnnichiha`→こんんにちは / `konnbannha`→こんんばんんは / `matcha`→まtちゃ
    ///   と、本実装が既に正しく処理しているケースを**回帰させる**。自前 n 処理の方が優秀。
    ///   詳細は memory `nuko-ime-romaji-library-eval` 参照。
    #[rstest]
    // 基本
    #[case("aiueo", "あいうえお")]
    ...
    #[case("kan'i", "かんい")]
    #[case("han'i", "はんい")] // 「範囲」: アポストロフィ方式
    #[case("hanni", "はんに")] // kanna→かんな と同じ規則の帰結 (n1個で ん、残り ni→に)
    #[case("hon'ya", "ほんや")]
    #[case("hen", "へん")]
    #[case("henn", "へん")]
    #[case("zannen", "ざんねん")]
