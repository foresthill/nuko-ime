...
=== 実機 smoke test チェックリスト (手で打って確認) ===
  1. 入力ソースを NukoIME に切替 (一度ことえり等に切ってから戻すと確実)
  2. 「にほんご」打鍵 → ローマ字がかなに変換される    (= 打てる)
  3. Space → 変換 / Enter → 確定 / Backspace → 1 文字削除
  4. 数キーを連打しても プロセスが crash しない
  5. (segmented 変換時) ←→ で文節フォーカスが動く
